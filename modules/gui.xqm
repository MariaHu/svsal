...
(: ####++++----
    ...
    - Meta fields for the html header
    - Header/Footer
...
(: create descriptive html header fields :)
...
declare %templates:wrap
    function gui:participantsTitle($node as node(), $model as map(*), $lang as xs:string?, $id as xs:string?) {
...
declare %private
    function gui:docSubjectname($id as xs:string) as xs:string? {
...
declare
    function gui:iiifUrl($node as node(), $model as map(*), $wid as xs:string*) as element() {
...
declare
    function gui:firstLink($node as node(), $model as map(*), $wid as xs:string?, $frag as xs:string?) as element(link)? {
...
declare
    function gui:prevLink($node as node(), $model as map(*), $wid as xs:string?, $frag as xs:string?) as element(link)? {
...
declare
    function gui:nextLink($node as node(), $model as map(*), $wid as xs:string?, $frag as xs:string?) as element(link)? {
    let $workId         := if ($wid) then sutil:normalizeId($wid) else $model("currentWorkId")
    let $htmlPath       := $config:html-root || "/" || $workId
    return  
        if (not(xmldb:collection-available($htmlPath))) then ()
...
(: Navigation:  create header and footer :)

(: Title of APP "Die Schule von Salamanca" :)
declare
    function gui:logo($node as node(), $model as map(*), $lang as xs:string) as element() {
    let $output := 
    ...
    if (doc($config:tei-works-root || "/" || sutil:normalizeId($wid) || ".xml")//tei:revisionDesc/@status eq "h_temporarily_suspended") then
        <div class="container">
            <div class="row-fluid alert alert-warning" style="margin-bottom:4px;">
                <p style="text-align: center;">{doc($config:tei-works-root || "/" || sutil:normalizeId($wid) || ".xml")//tei:revisionDesc//tei:change[@status eq "h_temporarily_suspended"][1]/replace(., '&#x0a;', ' ')}</p>
            </div>
        </div>
    else ()
};
...
declare
    ...
    if (doc($config:tei-works-root || "/" || sutil:normalizeId($wid) || ".xml")//tei:revisionDesc/@status eq "h_temporarily_suspended") then
        <span style="color:firebrick;">{doc($config:tei-works-root || "/" || sutil:normalizeId($wid) || ".xml")//tei:revisionDesc//tei:change[@status eq "h_temporarily_suspended"][1]/replace(., '&#x0a;', ' ')}</span>
    else ()
};
...
declare
    function gui:footer($node as node(), $model as map(*), $lang as xs:string) {

...
(: TODO: improvements: use id()/idref(), manually enforce lowercasing in g/@ref and char/@xml:id :)
    ...
    switch($mode)
        case 'orig'
        case 'snippets-orig' return
            let $glyph := $node/id(substring($node/@ref, 2)) (: remove leading '#' :)
        ...
        case 'edit' 
        case 'snippets-edit' return
            if ($node/@ref = ('#char017f', '#char0292')) then
                let $char := $node/id(substring($node/@ref, 2))
                return

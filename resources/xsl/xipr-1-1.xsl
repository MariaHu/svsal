...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xi="http://www.w3.org/2001/XInclude" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:xipr="http://dret.net/projects/xipr/" version="2.0">
    <xsl:template match="/*">
		<!-- if there is no other template handling the document element,
		     this template initiates xinclude processing at the document element of the input document. -->
        <xsl:apply-templates select="." mode="xipr"/>
    </xsl:template>
    <xsl:template match="@* | * | text()" mode="xipr">
    ...
    <xsl:template match="@* | * | text()" mode="xipr-internal">
        ...
        <xsl:copy>
            ...
            <xsl:apply-templates select="@* | * | text()" mode="xipr-internal"/>
        </xsl:copy>
    </xsl:template>
    <xsl:template match="xi:include" mode="xipr-internal">
		<!-- the two parameters are required for detecting inclusion loops, they contain the
		     complete history of href and xpointer attributes as sequences. -->
        <xsl:param name="uri-history" tunnel="yes"/>
        <xsl:param name="xpointer-history" tunnel="yes"/>
		<!-- REC: The children property of the xi:include element may include a single xi:fallback element;
		     the appearance of more than one xi:fallback element, an xi:include element, or any other element
        ...
        <xsl:variable name="include-uri" select="concat(replace(base-uri(/),'/[^/]*$',''), '/', @href)"/>
<!--        <xsl:variable name="include-uri" select="resolve-uri(@href, concat('file:',document-uri(/)))"/> -->
        <xsl:choose>
            <xsl:when test="@parse eq 'xml' or empty(@parse)">
                ...
                <xsl:if test="( index-of($uri-history, $include-uri ) = index-of($xpointer-history, string(@xpointer)) )">
					<!-- SPEC: When recursively processing an xi:include element, it is a fatal error to process another
					     xi:include element with an include location and xpointer attribute value that have already been
					     processed in the inclusion chain. -->
                    ...
                    <xsl:when test="doc-available($include-uri)">
                        ...
                        <xsl:choose>
                            <xsl:when test="empty(@xpointer)">
								<!-- SPEC: The inclusion target might be a document information item
								     (for instance, no specified xpointer attribute, or an XPointer specifically
								     locating the document root.) In this case, the set of top-level included
								     items is the children of the acquired infoset's document information item,
								     except for the document type declaration information item child, if one exists. -->
                            ...
                            <xsl:otherwise>
                                ...
                                <xsl:variable name="xpointer-node">
                                    <xsl:choose>
										<!-- xpointer uses a shorthand pointer (formerly known as barename),
										     NCName regex copied from the schema for schemas. -->
...
<!--                                            <xsl:copy-of select="xipr:include(id(@xpointer, $include-doc), $include-uri, @xpointer, $uri-history, $xpointer-history)"/> -->
                                            <xsl:copy-of select="xipr:include($include-doc//node()[@xml:id = current()/@xpointer], $include-uri, @xpointer, $uri-history, $xpointer-history)"/>
                                        </xsl:when>
										<!-- xpointer uses the element() scheme; regex derived from XPointer element()
										     scheme spec: http://www.w3.org/TR/xptr-element/#NT-ElementSchemeData
										     (NCName regex copied from the schema for schemas). -->
                                        <xsl:when test="matches(@xpointer, '^element\([\i-[:]][\c-[:]]*((/[1-9][0-9]*)+)?|(/[1-9][0-9]*)+\)$')">
                                            ...
                                            <xsl:choose>
                                                <xsl:when test="not(contains($element-pointer, '/'))">
													<!-- the pointer is a simple id, which can be located using the id()
													     function. -->
<!--                                                    <xsl:copy-of select="xipr:include(id($element-pointer, $include-doc), $include-uri, @xpointer, $uri-history, $xpointer-history)"/> -->
                                                    <xsl:copy-of select="xipr:include($include-doc//node()[@xml:id = current()/@xpointer], $include-uri, @xpointer, $uri-history, $xpointer-history)"/>
                                                </xsl:when>
                                                <xsl:otherwise>
													<!-- child sequence evaluation starts from the root or from an
													     element identified by a NCName. -->
<!--                                                    <xsl:copy-of select="xipr:include(xipr:child-sequence( if ( starts-with($element-pointer, '/') ) then $include-doc else id(substring-before($element-pointer, '/'), $include-doc), substring-after($element-pointer, '/')), $include-uri, @xpointer, $uri-history, $xpointer-history)"/> -->
                                                    <xsl:copy-of select="xipr:include(xipr:child-sequence( if ( starts-with($element-pointer, '/') ) then                                                                                                                 $include-doc else                                                                                                                                                                                 $include-doc/node()[@xml:id = substring-before($element-pointer, '/')], substring-after($element-pointer, '/')), $include-uri, @xpointer, $uri-history, $xpointer-history)"/>
                                                </xsl:otherwise>
                                            </xsl:choose>
                                        </xsl:when>
                                        <xsl:otherwise>
